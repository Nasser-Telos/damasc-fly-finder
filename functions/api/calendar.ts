...
export const onRequestPost: PagesFunction<Env> = async ({ request, env }) => {
  ...
  if (outbound_date_start && !/^\d{4}-\d{2}-\d{2}$/.test(outbound_date_start)) {
    return errorResponse('Invalid outbound_date_start: must be YYYY-MM-DD', 400);
  }
  if (outbound_date_end && !/^\d{4}-\d{2}-\d{2}$/.test(outbound_date_end)) {
    return errorResponse('Invalid outbound_date_end: must be YYYY-MM-DD', 400);
  }
